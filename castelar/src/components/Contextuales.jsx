...
import { getContextuales } from '../redux/actions/index';
...
const addTerapias = async (payload, id) => {
	try {
		...
		await updateDoc(ref, { contextual: payload });
...
const Contextuales = ({setOpenFiliatorio, registro}) => {

  const [terapias, setTerapias] = React.useState('');
	const dni = useSelector((state) => state.pacienteActual.filiatorios.dni);
	const paciente = useSelector((state) => state.pacienteActual.contextual);
  ...
  return (
    <>
    <Encabezado titulo = 'Terapias Contextuales' />
