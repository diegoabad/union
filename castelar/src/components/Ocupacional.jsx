...
import { getOcupacional } from '../redux/actions/index';
...
const addTerapias = async (payload, id) => {
	try {
		...
		await updateDoc(ref, { ocupacional: payload });
...
const Ocupacional = ({setOpenFiliatorio, registro}) => {

  const [terapias, setTerapias] = React.useState('');
	const dni = useSelector((state) => state.pacienteActual.filiatorios.dni);
	const paciente = useSelector((state) => state.pacienteActual.ocupacional);
  ...
  return (
    <>
    <Encabezado titulo = 'Terapia Ocupacional' />
