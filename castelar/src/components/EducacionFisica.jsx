...
import { getEdFisica } from '../redux/actions/index';
...
const addTerapias = async (payload, id) => {
	try {
		...
		await updateDoc(ref, { ed_fisica: payload });
...
const EducacionFisica = ({ setOpenFiliatorio, registro }) => {

  const [terapias, setTerapias] = React.useState('');
	const dni = useSelector((state) => state.pacienteActual.filiatorios.dni);
	const paciente = useSelector((state) => state.pacienteActual.ed_fisica);
  ...
  return (
    <>
    <Encabezado titulo = 'Educación Física' />
...
export default EducacionFisica