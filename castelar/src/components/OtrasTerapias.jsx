...
import { getOtrasTerapias } from '../redux/actions/index';
...
const addTerapias = async (payload, id) => {
	try {
		...
		await updateDoc(ref, { otras: payload });
...
const OtrasTerapias = ({setOpenFiliatorio, registro}) => {

  const [terapias, setTerapias] = React.useState('');
	const dni = useSelector((state) => state.pacienteActual.filiatorios.dni);
	const paciente = useSelector((state) => state.pacienteActual.otras);
        ...
        data = paciente.map((item, index) => {
          if (index === registro) {
			...
			if (result) {
				...
				dispatch(getOtrasTerapias(data));
  ...
  return (
    <>
    <Encabezado titulo = 'Terapias Varias' />
    <Paper className={classes.pagecontent} spacing={2}>
      <Grid container spacing={2}>

        <Grid item xs={12}>
        {editar ?
          <Evolucion terapias = {terapias} setTerapias = {setTerapias} paciente = {paciente[registro]}/>
          : <Evolucion terapias = {terapias} setTerapias = {setTerapias} paciente = {false}/>}
        </Grid>
