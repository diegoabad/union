import React,{useState} from 'react';
import s from './Professional.module.css';
import { NavLink } from 'react-router-dom';
import { filterProfessional } from '../../functions/filterProfessional';

export default function NavProfessional({setPros,pros}) {
const [focus,setFocus] = useState(false)
	function handleSearch (e){
		const professionals = filterProfessional(pros,e.target.value.toLowerCase().trim())
		setPros(professionals);
	}

	

	return (
		<div className={s.NavProfessionalContainer}>
			<NavLink className={s.navLinks} to='/'>
				<div className={s.NavProfessionalReturnContainer}>
					<img
						alt='volver a atras'
						className={s.off}
						src='data:image/png;base64,'
					/>
					<p>Volver</p>
				</div>
				
				</NavLink>
				
				<div className={focus ? `${s.containerSearch} ${s.focusSearch}` : s.containerSearch}>
				<img src="data:image/png;base64,"/>
					<input onFocus={()=> setFocus(true)} onBlur={()=>setFocus(false)} className={s.search} onChange={(e)=>handleSearch(e)} placeholder='Busca profesional por nombre o apellido'/>
				</div>
				
		</div>
	);
}
