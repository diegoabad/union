...
import Encabezado from './evaluaciones/Headers.jsx'
import Evaluacion from './evaluaciones/EvaluacionTerapia.jsx'
...
import { evaluacion } from './controls/evaluacion';
...
const Admisiones = ({propiedad, setOpenFiliatorio, registro, terapia}) => {
  ...
  const editar = useSelector((state) => state.editarFiliatorio)
	...
	const handleClick = async () => {
		const control = evaluacion(terapias);
    ...
    else setTerapias({...terapias, idProfesional: paciente[registro].idProfesional, nombreCompletoProfesional: paciente[registro].nombreCompletoProfesional, fechaCreacion: paciente[registro].fechaCreacion.seconds? new Date(paciente[registro].fechaCreacion.seconds * 1000) : new Date(paciente[registro].fechaCreacion), type: paciente[registro].type});
	}, []);
  ...
  return (
    <>
    <Encabezado titulo = {terapia.toUpperCase()} />
    <Paper className={classes.pagecontent} spacing={2}>
      <Grid container spacing={2}>

        <Grid item xs={12}>
          <Evaluacion terapias = {terapias} setTerapias = {setTerapias} paciente = {editar ? paciente[registro] : false}nombreTerapia = {terapia.toLowerCase()}/>
        </Grid>
